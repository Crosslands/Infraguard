...
{
    public partial class ReportesLogs : Form
    {
        ...
        private async void ReportesLogsForm_Load(object sender, EventArgs e)
        {
            await LoadReportesLogsAsync();
        }
        ...
        // Método para cargar los datos de la tabla `reporteslogs`
        private async Task LoadReportesLogsAsync()
        {
            ...
            try
            {
                var response = await httpClient.GetAsync($"{SupabaseUrl}/rest/v1/reporteslogs");
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    var reportesLogs = JsonSerializer.Deserialize<List<Dictionary<string, object>>>(json);

                    if (reportesLogs != null)
                    {
                        // Convertir a DataTable para el DataGridView
                        var dataTable = ConvertToDataTable(reportesLogs);
                        dgvReportesLogs.DataSource = dataTable;
                    }
                }
                else
                {
                    MessageBox.Show($"Error al cargar reportes logs: {response.StatusCode}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Error al cargar reportes logs: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
